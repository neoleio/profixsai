...
async function repairRequest(req, res) {
  ...
  const rawDate = sanitizeText(req.body?.preferred_date, 10);
  ...
  const rows = await sql`
    insert into repair_requests (full_name, contact_number, email, device_description, problem_description, preferred_date)
    values (${full_name}, ${contact_number}, ${email}, ${device_description}, ${problem_description}, ${preferred_date})
    returning id
  `;
