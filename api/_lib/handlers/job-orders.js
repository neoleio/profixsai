...
async function createJobOrder(req, res) {
  ...
  const rows = await sql`
    insert into job_orders (
      ...
      parts_required, technician_name, repair_notes, estimated_cost, final_cost, capital_cost, profit,
      ...
      ${sanitizeText(b.parts_required, 1000)}, ${sanitizeText(b.technician_name, 150)}, ${sanitizeText(b.repair_notes, 2000)},
  ...
  if (Array.isArray(b.items) && b.items.length) {
    try {
      await saveItems(jobOrder.id, b.items);
    } catch (err) {
      // The job order itself is already saved at this point — don't throw
      // and make the admin think nothing happened. Surface it as a warning
      // instead so they can still proceed and retry the items separately.
      console.error("Error saving job order items:", err);
      itemsWarning = "The job order was created, but the items list couldn't be saved. Add them from the job order's detail page.";
    }
  }

  await logAction({ userId: req.session.sub, action: `Created job order ${job_no}`, entity: "job_orders", entityId: jobOrder.id });
  res.status(201).json({ jobOrder, itemsWarning });
}

// Shared by create and the items sub-route: full replace (delete + re-insert)
// is simplest here since the form always sends the complete current list.
async function saveItems(jobOrderId, items) {
  await sql`delete from job_order_items where job_order_id = ${jobOrderId}`;
  const clean = items
    .slice(0, 50)
    .map((it) => ({
      part_name: sanitizeText(it.part_name || it.description, 200),
      quantity: Math.max(1, Math.round(Number(it.quantity ?? it.qty) || 1)),
      unit_cost: Math.max(0, Number(it.unit_cost ?? it.amount) || 0)
    }))
    .filter((it) => it.part_name);
  for (const it of clean) {
    await sql`
      insert into job_order_items (job_order_id, part_name, quantity, unit_cost)
      values (${jobOrderId}, ${it.part_name}, ${it.quantity}, ${it.unit_cost})
    `;
  }
  return clean;
}
...
async function updateJobOrder(req, res, id) {
  ...
  const rows = await sql`
    update job_orders set
      ...
      technician_name = coalesce(${!isTechnician && b.technician_name !== undefined ? sanitizeText(b.technician_name, 150) : null}, technician_name),
...
async function updateItems(req, res, id) {
  if (!["admin", "staff"].includes(req.session.role)) return res.status(403).json({ error: "You do not have permission to do that." });
  const existing = await sql`select id from job_orders where id = ${id}`;
  if (!existing[0]) return res.status(404).json({ error: "Job order not found." });
  const items = Array.isArray(req.body?.items) ? req.body.items : [];
  await saveItems(id, items);
  const rows = await sql`select * from job_order_items where job_order_id = ${id}`;
  await logAction({ userId: req.session.sub, action: "Updated job order items", entity: "job_orders", entityId: id });
...
export default async function handleJobOrders(req, res, rest) {
  ...
  if (rest.length === 2 && sub === "items") {
    if (req.method === "PUT") return updateItems(req, res, id);
    return methodNotAllowed(res, ["PUT"]);
  }
