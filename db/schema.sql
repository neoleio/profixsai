...
create table if not exists services (
  ...
  image_url text,                       -- optional photo (data URL or http link); falls back to icon when empty
...
create table if not exists hero_slides (
  id uuid primary key default gen_random_uuid(),
  image_url text not null,              -- data URL (admin upload) or http/relative path (seeded assets)
  alt_text text,
  sort_order int not null default 0
);
...
create table if not exists job_orders (
  ...
  technician_name text,                 -- free-typed technician name (admin no longer picks from a fixed account list)
  repair_notes text,
...
insert into hero_slides (image_url, alt_text, sort_order) values
...
create table if not exists repair_requests (
  ...
  preferred_date date,                  -- customer's preferred drop-off/service date
