...
import { ArrowLeft, RotateCcw, Inbox, Plus, Minus } from "lucide-react";
...
const emptyRepair = {
  ...
  technician_name: "", repair_notes: "", estimated_cost: "", expected_completion: "", warranty_info: ""
};
const emptyItem = () => ({ description: "", quantity: 1, unit_cost: "" });
const DEVICE_TYPES = ["Smartphone", "Laptop", "Tablet", "Desktop", "GPU", "Printer", "Other"];
...
export default function NewJobOrder() {
  ...
  async function onSubmit(e) {
    ...
    try {
      ...
      const { jobOrder, itemsWarning } = await api.post("/job-orders", {
      ...
      navigate(`/admin/job-orders/${jobOrder.id}`, { state: { itemsWarning } });
    } catch (err) {
      ...
      {sourceRequest && (
        <div className="flex items-start gap-3 bg-brand-blue/10 border border-brand-blue/20 rounded-lg p-4 mt-4 text-sm">
          ...
          <p className="text-ink/70">
            ...
            {new Date(sourceRequest.created_at).toLocaleDateString()}
            {sourceRequest.preferred_date && (
              <> — customer's preferred date: <strong>{formatDate(sourceRequest.preferred_date)}</strong></>
      ...
      <form onSubmit={onSubmit} className="mt-8 flex flex-col gap-8">
        ...
        <section className="card p-6">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            ...
            <Field label="Technician Assigned"><input className="input" placeholder="Type a technician's name" value={repair.technician_name} onChange={(e) => setRepair((r) => ({ ...r, technician_name: e.target.value }))} /></Field>
