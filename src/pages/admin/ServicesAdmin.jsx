...
const emptyForm = { name: "", category: "", description: "", icon: "wrench", image_url: "" };
...
export default function ServicesAdmin() {
  ...
  async function updateImage(s, image_url) {
    setServices((prev) => prev.map((svc) => (svc.id === s.id ? { ...svc, image_url } : svc)));
    await api.put(`/services?id=${s.id}`, { image_url });
  }
      ...
      {showForm && (
        <form onSubmit={addService} className="card p-6 mt-6 grid sm:grid-cols-2 gap-4">
          ...
          <div className="sm:col-span-2">
            <p className="text-xs font-semibold text-ink/50 mb-2">Photo (optional — shown instead of the icon on the site)</p>
            <ImageUpload value={form.image_url} onChange={(image_url) => setForm((f) => ({ ...f, image_url }))} label="photo" />
          </div>
      ...
      <div className="grid sm:grid-cols-2 gap-4 mt-6">
        {services.map((s) => (
          <div key={s.id} className={`card p-5 ${!s.is_active ? "opacity-50" : ""}`}>
            ...
            <div className="mt-3">
              <ImageUpload value={s.image_url} onChange={(image_url) => updateImage(s, image_url)} label="photo" />
            </div>
